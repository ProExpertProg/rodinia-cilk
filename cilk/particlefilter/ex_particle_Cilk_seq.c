...
void getneighbors(int * se, int numOnes, double * neighbors, int radius){
  printf("getneighbors: se %p, numOnes %d, neighbors %p, radius %d\n",
	 se, numOnes, neighbors, radius);
	int x, y;
	int neighY = 0;
	int center = radius - 1;
	int diameter = radius*2 -1;
...
void particleFilter(int * I, int IszX, int IszY, int Nfr, int * seed, int Nparticles){
	...
	for(x = 0; x < diameter; x++){
		for(y = 0; y < diameter; y++){
			if(disk[x*diameter + y] == 1)
				countOnes++;
			else if (disk[x*diameter + y])
			  printf("x %d, y %d, disk %d\n", x, y, disk[x*diameter + y]);
		}
	}
	...
	for(k = 1; k < Nfr; k++){
		...
		cilk_for(int x = 0; x < Nparticles; x++){
			...
			//calc ind
			cilk_for(int y = 0; y < countOnes; y++){
		...
		// TBS NOTE: Could be parallelized as a prefix-sum computation.
		CDF[0] = weights[0];
		...
		cilk_for(int j = 0; j < Nparticles; j++){
