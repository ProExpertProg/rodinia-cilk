...
double pgain(long x, Points *points, double z, long int *numcenters, int pid, pthread_barrier_t* barrier)
{
  ...
  cilk::reducer<cilk::op_add<double>> cost_of_opening_x_r(0.0);
...
//	#pragma omp parallel for 
  ...
  cilk_for (int i = k1; i < k2; i++ ) {
    ...
    if ( x_cost < current_cost ) {
      ...
      *cost_of_opening_x_r += x_cost - current_cost;			
    } else {
  ...
  double cost_of_opening_x = cost_of_opening_x_r.get_value();
  for ( int i = k1; i < k2; i++ ) {
    if( is_center[i] ) {
      double low = z;
