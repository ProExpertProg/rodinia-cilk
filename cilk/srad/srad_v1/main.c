...
int main(int argc, char *argv []){
    ...
    for (iter=0; iter<niter; iter++){										// do for the number of iterations input parameter
		...
		cilk_for (int j=0; j<Nc; j++) {												// do for the range of columns in IMAGE

            for (int i=0; i<Nr; i++) {											// do for the range of rows in IMAGE 

                // current index/pixel
                int k = i + Nr*j;												// get position of current element
                fp Jc = image[k];													// get value of the current element
                ...
                fp G2 = (dN[k]*dN[k] + dS[k]*dS[k]								// gradient (based on derivatives)
                    + dW[k]*dW[k] + dE[k]*dE[k]) / (Jc*Jc);

                // normalized discrete laplacian (equ 54)
                fp L = (dN[k] + dS[k] + dW[k] + dE[k]) / Jc;					// laplacian (based on derivatives)

                // ICOV (equ 31/35)
                fp num  = (0.5*G2) - ((1.0/16.0)*(L*L)) ;						// num (based on gradient and laplacian)
                fp den  = 1 + (.25*L);											// den (based on laplacian)
                fp qsqr = num/(den*den);										// qsqr (based on num and den)
        ...
        cilk_for (int j=0; j<Nc; j++) {												// do for the range of columns in IMAGE
            ...
            for (int i=0; i<Nr; i++) {											// do for the range of rows in IMAGE
                ...
                fp cN = c[k];													// north diffusion coefficient
                fp cS = c[iS[i] + Nr*j];										// south diffusion coefficient
                fp cW = c[k];													// west diffusion coefficient
                fp cE = c[i + Nr*jE[j]];										// east diffusion coefficient

                // divergence (equ 58)
                fp D = cN*dN[k] + cS*dS[k] + cW*dW[k] + cE*dE[k];				// divergence
